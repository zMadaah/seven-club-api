...
export async function asaasWebhookRoutes(app: FastifyInstance) {
  app.post('/webhooks/asaas', async (request, reply) => {
    ...
    if (!subscriptionId) return reply.code(200).send({ received: true }); // cobrança avulsa, não é de assinatura
    ...
    if (rows.length === 0) return reply.code(200).send({ received: true });
